...
{
    public interface IEventLoggerLocal
    {
        void RecordEvent(String eventText, bool force = false);
    }
      ...
      public void Subscribe(string eventOperation)
      {
         lock(typeof(SubscriptionManager<T>))
         {
            ...
            if (EventLogger != null)
            {
                ...
                EventLogger.RecordEvent("Subscribe: " + endpointProperty.Address + ":" + endpointProperty.Port + " - for Operation: " + eventOperation);
            }
            ...
            else
            {
             ...
             //DMF - Send available events to new subscribers
            if (PublishService<T>.AvailableEventListArgs != null)
            {
                T[] arrayOfOne = new T[1];
                arrayOfOne[0] = subscriber;
                PublishService<T>.Publish(arrayOfOne, false, "AvailableEventList", PublishService<T>.AvailableEventListArgs);
            }
         }
      }
         ...
         lock(typeof(SubscriptionManager<T>))
         {
            ...
            if (EventLogger != null)
            {
                ...
                RemoteEndpointMessageProperty endpointProperty =
                    messageProperties[RemoteEndpointMessageProperty.Name]
                    as RemoteEndpointMessageProperty;
                EventLogger.RecordEvent("Unsubscribe: " + endpointProperty.Address + ":" + endpointProperty.Port + " - for Operation: " + eventOperation);
            }
