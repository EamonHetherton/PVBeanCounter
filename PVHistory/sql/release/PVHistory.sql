...
BEGIN
    RAISERROR(N'You cannot deploy this update script to target BLACKSATIN\SQLEXPRESS. The database for which this script was built, PVHistory, does not exist on this server.', 16, 127) WITH NOWAIT
    RETURN
END

GO

IF (@@servername != 'BLACKSATIN\SQLEXPRESS')
BEGIN
    RAISERROR(N'The server name in the build script %s does not match the name of the target server %s. Verify whether your database project settings are correct and whether your build script is up to date.', 16, 127,N'BLACKSATIN\SQLEXPRESS',@@servername) WITH NOWAIT
    RETURN
END
